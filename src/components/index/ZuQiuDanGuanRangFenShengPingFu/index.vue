<template>
  <div class="components ZuQiuDanGuanRangFenShengPingFu">
    <ul>
      <li v-for="(item, index) in PageData" :key="index">
        ...
        <div class="Detail">
          <ZqdghhtzRangFenShengPingFu :Id="item.matchId" :ListId="item.matchWeek" :HomeTeamName="item.homeTeamName" :AwayTeamName="item.awayTeamName" :HomeTeamId="item.homeTeamId" :AwayTeamId="item.awayTeamId" :Index="index" :WdlsData="item.playOdds['ft-wdls']" @SelectItem="SelectItem"></ZqdghhtzRangFenShengPingFu>
        </div>
      </li>
    </ul>
...
/*
  页面名称：足球-过关-混合投注
*/
import ZqdghhtzRangFenShengPingFu from '../ZqdghhtzRangFenShengPingFu'
export default {
  name: 'ZuQiuDanGuanRangFenShengPingFu',
  props: {
    Data: {
      default: []
    }
  ...
  components: {
    ZqdghhtzRangFenShengPingFu
  },
  created () {
    let Num = 0
    this.Data.map((item) => {
      if (item.playOdds['ft-wdls'] !== undefined) {
        Num += 1
      }
    })
