                ...
                <ul>
                  <li v-for="(item, index) in LotterySportRes" :key="index" v-if="item.playOdds['ft-wdl'] !== undefined" class="flex-h">
                    ...
                    <div class="flex-item flex-h">
                      ...
                      <div class="flex-item">
                        <span class="Small">{{!item.playOdds['ft-wdl'][0].isWinning ? '胜' : '负'}}</span>
                ...
                <ul>
                  <li v-for="(item, index) in LotterySportRes" :key="index" v-if="item.playOdds['ft-wdls'] !== undefined" class="flex-h">
                    ...
                    <div class="flex-item flex-h">
                      ...
                      <div class="flex-item">
                        <span class="Small">{{!item.playOdds['ft-wdls'][0].isWinning ? `让胜（${item.spreadPoints}）` : `让负（${item.spreadPoints}）`}}</span>
                        <span class="Big">{{item.wholeScore}}</span>
                      </div>
                      <div class="flex-item">
                        <span>{{item.homeTeamName}}</span>
                ...
                <ul>
                  <li v-for="(item, index) in LotterySportRes" :key="index" v-if="item.playOdds['ft-points'] !== undefined" class="flex-h">
                    ...
                    <div class="flex-item flex-h">
                      ...
                      <div class="flex-item">
                        <span class="Small" v-for="(subItem, subIndex) in item.playOdds['ft-points']" :key="subIndex" v-if="subItem.isWinning">{{subItem.name.substring(0, 1) === '胜' ? '负' : '胜'}}{{subItem.name.substring(1)}}</span>
                        <span class="Big">{{item.wholeScore}}</span>
                      </div>
                      <div class="flex-item">
                        <span>{{item.homeTeamName}}</span>
