<template>
  <div class="Page">
    <MainPageContainer :GoBack="() => { $router.replace({ name: 'LanQiu' }) }" :ShowMainHeader="true" :ShowMainHeaderBack="true" MainHeaderTitle="篮球">
      <div class="PageContainer flex-v">
        ...
        <div class="Bottom">
          ...
          <a :class="{'active': CurrentStoreId !== null && LanQiuCart.length && CurrentPassCode !== null}" @click="ConfirmBetting">立即购买</a>
        </div>
...
export default {
  ...
  methods: {
    ...
    ToGetAllParts () { // 枚举投注
      ...
      if (this.CurrentPassCode === null) {
        this.FootballKeys.passes.map((item) => {
          if (item.unitNum > 1 && item.amount === 1 && item.unitNum <= this.PageList.length) {
